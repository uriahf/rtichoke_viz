...
// src/spec/v2/prediction-distribution.ts
var PredictionDistributionBinSchema = Type.Object({
  evaluationId: Type.String(),
...
var PredictionDistributionSpecSchema = Type.Object({
  schemaVersion: Type.Literal("2.0"),
...
function assertPredictionDistributionReferentialIntegrity(spec) {
  ...
  const binsByEvaluation = /* @__PURE__ */ new Map();
  ...
  const opsByEvaluation = /* @__PURE__ */ new Map();
  const opKeys = /* @__PURE__ */ new Set();
  for (const op of spec.operatingPoints) {
    ...
    const opKey = `${op.evaluationId}\0${op.type}\0${op.value}`;
    if (opKeys.has(opKey)) {
      throw new Error(
    ...
    opKeys.add(opKey);
    if (!Number.isFinite(op.value) || op.value < 0 || op.value > 1 || !Number.isFinite(op.cutoff) || op.cutoff < 0 || op.cutoff > 1 || !Number.isFinite(op.realizedPpcr) || op.realizedPpcr < 0 || op.realizedPpcr > 1) {
      throw new Error(
  ...
  for (const evalId of evaluationIds) {
    ...
    const bin0 = bins[0];
    if (bin0.lower !== 0 || bin0.upper !== 0 || bin0.includeLower !== true || bin0.includeUpper !== true) {
      throw new Error(`first bin for evaluation ${evalId} must be [0, 0]`);
    }
    const binUppers = /* @__PURE__ */ new Set([0]);
    let totalCount = bin0.nPositive + bin0.nNegative;
    if (!Number.isInteger(bin0.nPositive) || bin0.nPositive < 0 || !Number.isInteger(bin0.nNegative) || bin0.nNegative < 0) {
      throw new Error(`invalid bin counts for evaluation ${evalId}`);
    }
    for (let i = 1; i < bins.length; i++) {
      const bin = bins[i];
      if (!Number.isFinite(bin.lower) || bin.lower < 0 || bin.lower > 1 || !Number.isFinite(bin.upper) || bin.upper < 0 || bin.upper > 1) {
        throw new Error(`invalid bin bounds for evaluation ${evalId}`);
      }
      if (!Number.isInteger(bin.nPositive) || bin.nPositive < 0 || !Number.isInteger(bin.nNegative) || bin.nNegative < 0) {
        throw new Error(`invalid bin counts for evaluation ${evalId}`);
      }
...
function mergeTheme(options) {
  ...
  return {
    ...
    predictionDistribution: {
      ...
      ...custom8.predictionDistribution
    }
  };
}
...
function preparePredictionDistributionPlotData(spec, evalId, dim, currentValue, digits) {
  const ops = spec.operatingPoints.filter((op) => op.evaluationId === evalId && op.type === dim).sort((a2, b) => a2.value - b.value);
  ...
  const evalBins = spec.bins.filter((bin) => bin.evaluationId === evalId).sort((a2, b) => a2.lower - b.lower);
  let totalN = 0;
...
function renderPredictionDistribution(spec, options = {}) {
  ...
  const getOperatingPoints = (evalId, dim) => {
    return spec.operatingPoints.filter((op) => op.evaluationId === evalId && op.type === dim).sort((a2, b) => a2.value - b.value);
  };
  const initialDims = getAvailableDimensions(currentEvalId);
  let currentDim = spec.operatingPoint?.dimension && initialDims.includes(spec.operatingPoint.dimension) ? spec.operatingPoint.dimension : initialDims[0] ?? "probability_threshold";
  const getValuesFor = (evalId, dim) => {
    const ops = getOperatingPoints(evalId, dim);
    return [...new Set(ops.map((op) => op.value))].sort((a2, b) => a2 - b);
  };
  const pickBestValue = (evalId, dim, preferredVal) => {
    const vals = getValuesFor(evalId, dim);
    if (vals.length === 0) return 0;
    if (preferredVal !== void 0 && vals.includes(preferredVal)) {
      return preferredVal;
    }
  ...
  let currentColorMode = "confusion_matrix_cell";
  let currentConditioning = "predicted_positives";
  const container = document.createElement("div");
  ...
  if (spec.evaluations.length > 1) {
    ...
    for (const evalSpec of spec.evaluations) {
      ...
      opt.textContent = evalSpec.label ?? evalSpec.model ?? evalSpec.population ?? evalSpec.id;
  ...
  const updateDimSelectOptions = () => {
    ...
    for (const dim of available) {
      const opt = document.createElement("option");
      opt.value = dim;
      opt.textContent = dim === "probability_threshold" ? "Probability threshold" : "PPCR";
  ...
  const createLegendItem = (label, color3) => {
    ...
    const lineSpan = document.createElement("span");
    lineSpan.className = "rtichoke-legend-line";
    lineSpan.style.backgroundColor = color3;
  ...
  const updateChart = () => {
    ...
    const dimLabelText = currentDim === "probability_threshold" ? "Probability threshold" : "PPCR";
    ...
    const bgRegions = [];
    const highlightColor = currentColorMode === "confusion_matrix_cell" ? cellColors.tp : theme.predictionDistribution.observedPositive;
    ...
    const fillKey = currentColorMode === "confusion_matrix_cell" ? "classificationCell" : "category";
    const colorDomain = currentColorMode === "confusion_matrix_cell" ? ["TP", "FP", "TN", "FN"] : ["Observed Positives", "Observed Negatives"];
    const colorRange = currentColorMode === "confusion_matrix_cell" ? [cellColors.tp, cellColors.fp, cellColors.tn, cellColors.fn] : [
      theme.predictionDistribution.observedPositive,
      theme.predictionDistribution.observedNegative
    ];
    ...
    const createMetricBadge = (lbl, val) => {
  ...
  conditioningSelect.addEventListener("change", () => {
    currentConditioning = conditioningSelect.value;
    updateChart();
  });
  slider.addEventListener("input", () => {
    const availableValues = getValuesFor(currentEvalId, currentDim);
...
// fixtures/v2/performance-table.json
var performance_table_default = {
  schemaVersion: "2.0",
  type: "performance_table",
  title: "Performance table",
  evaluations: [
    { id: "eval-model-a-pop-a", model: "model-a", population: "pop-a", label: "Model A / Population A" },
    { id: "eval-model-b-pop-a", model: "model-b", population: "pop-a", label: "Model B / Population A" },
    { id: "eval-model-a-pop-b", model: "model-a", population: "pop-b", label: "Model A / Population B" },
    { id: "eval-unknown-pop-c", population: "pop-c", label: "Population C" }
  ],
  metrics: [
  ...
  rows: [
    {
      evaluationId: "eval-model-a-pop-a",
      operatingPoint: { type: "probability_threshold", value: 0.25 },
      values: [
        { metricId: "sensitivity", estimate: 0.82 },
        { metricId: "specificity", estimate: 0.74 },
        { metricId: "ppv", estimate: 0.4 },
        { metricId: "net_benefit", estimate: 0 }
      ]
    },
    {
      evaluationId: "eval-model-b-pop-a",
      operatingPoint: { type: "ppcr", value: 0.2 },
      values: [
        { metricId: "sensitivity", estimate: 0.79 },
        { metricId: "specificity", estimate: 0.77 },
        { metricId: "ppv", estimate: null }
      ]
    },
    {
      evaluationId: "eval-model-a-pop-b",
      operatingPoint: { type: "probability_threshold", value: 0.25 },
      values: [
        { metricId: "sensitivity", estimate: 0.82 },
        { metricId: "specificity", estimate: 0.74 }
    ...
    {
      evaluationId: "eval-unknown-pop-c",
      horizon: 365,
      operatingPoint: { type: "probability_threshold", value: 0.25 },
      context: {
      ...
      values: [{ metricId: "sensitivity", estimate: 0.7 }]
    },
    {
      evaluationId: "eval-unknown-pop-c",
      horizon: 730,
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          spec: interventions_avoided_single_default
        }
      ]
    },
    {
          ...
          components: [
            {
              ...
              spec: performance_table_default
            }
          ]
        },
        {
...
// fixtures/v2/gains-time.json
var gains_time_default = {
  schemaVersion: "2.0",
  type: "gains",
  evaluations: [{ id: "eval-a", model: "Model A", population: "Population A" }],
  series: [
    { id: "series-a-5", evaluationId: "eval-a", horizon: 5, display: { label: "5 years", group: "Model A", role: "evaluation" } },
    { id: "series-a-10", evaluationId: "eval-a", horizon: 10, display: { label: "10 years", group: "Model A", role: "evaluation" } }
  ],
  data: [
    { seriesId: "series-a-5", cutoff: 0.5, ppcr: 0.35, sensitivity: 0.75 },
    { seriesId: "series-a-10", cutoff: 0.5, ppcr: 0.45, sensitivity: 0.8 }
  ],
  x: "ppcr",
  y: "sensitivity",
  xAxis: { label: "Predicted Positives (Rate)", domain: [0, 1] },
...
// fixtures/v2/lift-time.json
var lift_time_default = {
  schemaVersion: "2.0",
  type: "lift",
  evaluations: [
    { id: "eval-a", model: "Model A", population: "Population A", label: "Model A" }
  ...
  series: [
    {
      ...
      evaluationId: "eval-a",
      horizon: 5,
      display: { label: "Model A (t=5)", group: "Model A", role: "model" }
    },
    {
      id: "series-a-10",
      evaluationId: "eval-a",
      horizon: 10,
      display: { label: "Model A (t=10)", group: "Model A", role: "model" }
    }
  ],
  data: [
    { seriesId: "series-a-5", cutoff: 0.5, ppcr: 0.2, lift: 5 },
    { seriesId: "series-a-5", cutoff: 0, ppcr: 1, lift: 1 },
    { seriesId: "series-a-10", cutoff: 0.5, ppcr: 0.4, lift: 2.5 },
    { seriesId: "series-a-10", cutoff: 0, ppcr: 1, lift: 1 }
  ],
  x: "ppcr",
  y: "lift",
  xAxis: { label: "Predicted Positives (Rate)", domain: [0, 1] },
...
// fixtures/v2/prediction-distribution-threshold.json
var prediction_distribution_threshold_default = {
  schemaVersion: "2.0",
  type: "prediction_distribution",
  title: "Threshold Golden Fixture",
  evaluations: [
    {
      id: "Model A",
      model: "Model A",
...
// fixtures/v2/prediction-distribution-ppcr-tie.json
var prediction_distribution_ppcr_tie_default = {
  schemaVersion: "2.0",
  type: "prediction_distribution",
  title: "PPCR Tie Golden Fixture",
  evaluations: [
    {
      id: "Model A",
      model: "Model A",
...
gainsHost.append(
  renderGainsV2({
    ...gains_shared_population_default,
    operatingPoint: { dimension: "ppcr" }
  })
);
...
liftHost.append(
  renderLiftV2({
    ...lift_shared_population_default,
    operatingPoint: { dimension: "ppcr" }
  })
);
