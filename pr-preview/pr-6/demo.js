...
function createScaleD(key, scale, transform2, channels, {
  ...
  if (symmetric) {
    ...
    const mindelta = mid - transform2.apply(min4);
    const maxdelta = transform2.apply(max3) - mid;
    if (mindelta < maxdelta) min4 = transform2.invert(mid - maxdelta);
    else if (mindelta > maxdelta) max3 = transform2.invert(mid + mindelta);
  }
