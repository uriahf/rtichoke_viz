...
render_roc_ggplot <- function(spec) {
  ...
  p <- ggplot(dat, aes(x = false_positive_rate, y = sensitivity, color = model, group = model)) +
...
render_roc_plotly <- function(spec) {
  ggplotly(
    render_roc_ggplot(spec),
    tooltip = c("model", "false_positive_rate", "sensitivity")
  ) |>
...
render_calibration_plotly <- function(spec) {
  main <- ggplotly(
    ...
    tooltip = c("model", "predicted", "observed", "events", "total")
  )
  ...
  models <- unique(dist$model)
  colors <- if (length(models) == 1) "black" else rtichoke_colors[seq_along(models)]

  hist <- plot_ly()
  for (i in seq_along(models)) {
    model <- models[[i]]
    d <- dist[dist$model == model, , drop = FALSE]
    hist <- add_bars(
      hist,
      data = d,
      x = ~midpoint,
